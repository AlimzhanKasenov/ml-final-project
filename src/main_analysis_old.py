...
def create_model_comparison_plots(results_df: pd.DataFrame, images_dir: str) -> None:
    ...
    plt.figure(figsize=(10, 5))
    sns.barplot(data=results_df, x="Модель", y="F1")
    plt.title("Сравнение моделей по F1-score")
    plt.xlabel("Модель")
    plt.ylabel("F1-score")
    plt.xticks(rotation=25)
    save_plot(os.path.join(images_dir, "09_сравнение_моделей_f1.png"))
...
def create_feature_importance_plot(model, feature_names, images_dir: str) -> pd.DataFrame:
    ...
    plt.xlabel("Важность")
    plt.ylabel("Признак")
    save_plot(os.path.join(images_dir, "10_важность_признаков.png"))
...
Лучшая модель по метрике ROC-AUC:
{best_model["Модель"]}

ROC-AUC лучшей модели:
{best_model["ROC_AUC"]:.4f}
